...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, EnMPDIoU=False, eps=1e-7):
    ...
    iou = inter / union
    if GIoU or DIoU or CIoU or EnMPDIoU:
        cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)  # convex (smallest enclosing box) width
        ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)  # convex height
        if CIoU or DIoU or EnMPDIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if DIoU:
                ...
                print("test")
                # 计算目标宽高
                iw = torch.abs(b1_x2 - b1_x1)
                ih = torch.abs(b1_y2 - b1_y1)
                ...
                # 创新部分：引入尺度自适应权重
                scale_weight = torch.exp(-torch.abs(torch.log(cw * ch / (iw * ih + 1e-6))))

                # 创新部分：引入边界敏感项
                boundary_sensitive = torch.abs(b2_x1 - b1_x1) + torch.abs(b2_y1 - b1_y1) + \
                                     torch.abs(b2_x2 - b1_x2) + torch.abs(b2_y2 - b1_y2)

                # 创新部分：结合尺度自适应权重和边界敏感项
                return iou - scale_weight * ((d12 / sq_sum) + (d22 / sq_sum)) - 0.1 * boundary_sensitive
